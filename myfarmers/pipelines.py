...
class MyfarmersPipeline:

    # Database setup
    conn = sqlite3.connect('myfarmers.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `myfarmers`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM myfarmers WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO myfarmers (date, title, link, content)"
